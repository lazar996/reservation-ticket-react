...
function SearchByReturn() {
   ...
   useEffect(()=>{
    ...
    getFlight.getFlightByDateAndAirPortReturn(returnFlight,destination,origin ).then(
        (response)=>{
  
          setFlightReturn(response.data);
   ...
   {flight.map((fl)=>(
    ...
    {flightReturn.map((flr)=>(
    
        <li key={flr.id}>{flr.id}</li>
            ))}
  ...
  return (
    ...
    <tbody>
        ...
        {flightReturn.map((flightDataReturn)=>
         <tr key={flightDataReturn.id}><td>{flightDataReturn.code}</td>
         <td>{flightDataReturn.price}</td>
         <td>{getFormattedDate2(flightDataReturn.departureDate)}</td>
         <td>{flightDataReturn.route.arrivalAirport.city}</td>
         <td>{flightDataReturn.route.departureAirport.city}</td>
